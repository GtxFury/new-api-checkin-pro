...
BALANCE_HASH_FILE = "balance_hash_ccode.txt"
CACHE_DIR = os.path.join("storage-states", "ccode")


def _load_accounts() -> list[AccountConfig] | None:
    accounts_str = os.getenv("ACCOUNTS_CCODE")
    if not accounts_str:
        print("❌ ACCOUNTS_CCODE environment variable not found")
        return None

    try:
        data = json.loads(accounts_str)
    except json.JSONDecodeError as e:
        print(f"❌ Failed to parse ACCOUNTS_CCODE as JSON: {e}")
        return None
    ...
    if isinstance(data, dict):
        ...
        print("❌ ACCOUNTS_CCODE must be a JSON object or array")
        return None
    ...
    for i, account in enumerate(accounts_data):
        ...
        # 默认强制 provider=ccode（也允许用户显式填写）
        account.setdefault("provider", "ccode")
...
async def main() -> int:
    print("🚀 ccode 有间公益自动签到脚本启动")
    print(f"🕒 执行时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    app_config = AppConfig.load_from_env()
    provider = app_config.get_provider("ccode")
    if not provider:
        print("❌ Provider 'ccode' 未加载：请通过 PROVIDERS 注入/覆盖该站点配置")
        return 1
    ...
    for i, account_config in enumerate(accounts):
        ...
        try:
            ...
            # ccode 预期使用 linux.do，但这里兼容多种认证方式的返回格式
            ok_any = False
    ...
    if need_notify and notification_lines:
        ...
        title = "ccode 签到成功" if success_count == total_count else "ccode 签到告警"
        print(content)
