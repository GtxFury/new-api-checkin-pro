...
BALANCE_HASH_FILE = "balance_hash_neb.txt"
CACHE_DIR = os.path.join("storage-states", "neb")


def _load_accounts() -> list[AccountConfig] | None:
    accounts_str = os.getenv("ACCOUNTS_NEB")
    if not accounts_str:
        print("❌ ACCOUNTS_NEB environment variable not found")
        return None

    try:
        data = json.loads(accounts_str)
    except json.JSONDecodeError as e:
        print(f"❌ Failed to parse ACCOUNTS_NEB as JSON: {e}")
        return None
    ...
    if isinstance(data, dict):
        ...
        print("❌ ACCOUNTS_NEB must be a JSON object or array")
        return None

    overridden = apply_linuxdo_cookies_override(accounts_data, accounts_env_key="ACCOUNTS_NEB")
    if overridden:
    ...
    for i, account in enumerate(accounts_data):
        ...
        # 默认强制 provider=neb（也允许用户显式填写）
        account.setdefault("provider", "neb")
...
async def main() -> int:
    ...
    provider = app_config.get_provider("neb")
    if not provider:
        print("❌ Provider 'neb' 未加载：请通过 PROVIDERS 注入/覆盖该站点配置")
        return 1
    ...
    if need_notify and notification_lines:
        ...
        title = "neb 签到成功" if success_count == total_count else "neb 签到告警"
        print(content)
