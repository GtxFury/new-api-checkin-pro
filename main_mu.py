...
BALANCE_HASH_FILE = "balance_hash_mu.txt"
CACHE_DIR = os.path.join("storage-states", "mu")


def _load_accounts() -> list[AccountConfig] | None:
    accounts_str = os.getenv("ACCOUNTS_MU")
    if not accounts_str:
        print("❌ ACCOUNTS_MU environment variable not found")
        return None

    try:
        data = json.loads(accounts_str)
    except json.JSONDecodeError as e:
        print(f"❌ Failed to parse ACCOUNTS_MU as JSON: {e}")
        return None
    ...
    if isinstance(data, dict):
        ...
        print("❌ ACCOUNTS_MU must be a JSON object or array")
        return None

    overridden = apply_linuxdo_cookies_override(accounts_data, accounts_env_key="ACCOUNTS_MU")
    if overridden:
    ...
    for i, account in enumerate(accounts_data):
        ...
        # 默认强制 provider=mu（也允许用户显式填写）
        account.setdefault("provider", "mu")
...
async def main() -> int:
    print("🚀 mu（Mu.API）自动签到脚本启动")
    print(f"🕒 执行时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    app_config = AppConfig.load_from_env()
    provider = app_config.get_provider("mu")
    if not provider:
        print("❌ Provider 'mu' 未加载：请通过 PROVIDERS 注入/覆盖该站点配置")
        return 1
    ...
    if need_notify and notification_lines:
        ...
        title = "mu 签到成功" if success_count == total_count else "mu 签到告警"
        print(content)
