...
class LikeCounter {
    constructor() {
        this.CONFIG = {
            STORAGE_KEY: 'linuxdo_likes_counter',
    ...
    async syncRemote() {
        ...
        try {
            ...
            const reactions = await this.fetchUserActions(username);
            ...
            // 获取当前用户的限额
            const userLimit = this.getDailyLimit();
            console.log(`[LikeCounter] 用户信任等级: ${this.currentUser?.trust_level}, 限额: ${userLimit}`);
            console.log(`[LikeCounter] 从 API 获取到 ${reactions.length} 条反应记录，去重后 ${dedupedTimestamps.length} 个不同帖子`);

            // 直接使用去重后的时间戳（完全替换本地数据，以API数据为准）
    ...
    async fetchUserActions(username) {
        ...
        console.log(`[LikeCounter] 开始获取 ${username} 的点赞历史 (user_actions API)...`);
        ...
        while (pages < 5) {
            try {
                ...
                if (hasOld || items.length < 50) {
                    console.log(`[LikeCounter] ${hasOld ? '遇到旧数据' : '数据不足50条'}，结束获取`);
                    break;
                }
                offset += 50;
                pages++;
...
class BrowseController {
    ...
    startTrustLevelMonitor() {
        ...
        console.log('等级监控已启动（60秒刷新一次，仅在可见时）');
    }
    ...
    setupButton() {
        ...
        this.likeCounterContainer.addEventListener('click', () => {
            if (this.likeCounter) {
                ...
                this.likeCounter.syncRemote().then(() => {
    ...
    createSliderRow(label, value, min, max, step, onChange) {
        ...
        sliderStyle.textContent = `
            .toggle-row input[type="range"]::-webkit-slider-thumb {
                -webkit-appearance: none;
                width: 16px;
                height: 16px;
                border-radius: 50%;
            ...
            .toggle-row input[type="range"]::-moz-range-thumb {
                width: 16px;
                height: 16px;
                border-radius: 50%;
                background: #ffd700;
    ...
    fetchRankingByPeriod(period) {
        return new Promise((resolve) => {
            fetch(`${BASE_URL}/leaderboard/1.json?period=${period}`, {
                credentials: 'include',
                headers: {
    ...
    fetchLeaderboardData() {
        return new Promise((resolve) => {
            ...
            Promise.all([
                ...
                // 总榜：获取总积分（当前点数）
                fetch('https://linux.do/leaderboard/1.json?period=all', {
                    credentials: 'include',
                    headers: { 'Accept': 'application/json' }
                }).then(r => r.ok ? r.json() : null).catch(() => null)
            ])
