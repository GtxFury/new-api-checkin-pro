#!/usr/bin/env python3
"""
佬友 (api.freestyle.cc.cd) 自动签到脚本（独立入口）
"""
...
BALANCE_HASH_FILE = "balance_hash_freestyle.txt"
CACHE_DIR = os.path.join("storage-states", "freestyle")


def _load_accounts() -> tuple[list[AccountConfig] | None, str | None]:
    accounts_str = os.getenv("ACCOUNTS_FREESTYLE")
    if not accounts_str:
        msg = "❌ ACCOUNTS_FREESTYLE environment variable not found"
        print(msg)
        return None, msg
    ...
    try:
        ...
        msg = f"❌ Failed to parse ACCOUNTS_FREESTYLE as JSON: {e}"
        print(msg)
        return None, msg
    ...
    if isinstance(data, dict):
        ...
        msg = "❌ ACCOUNTS_FREESTYLE must be a JSON object or array"
        print(msg)
        return None, msg

    overridden = apply_linuxdo_cookies_override(accounts_data, accounts_env_key="ACCOUNTS_FREESTYLE")
    if overridden:
    ...
    for i, account in enumerate(accounts_data):
        ...
        # 默认强制 provider=freestyle（也允许用户显式填写）
        account.setdefault("provider", "freestyle")
...
async def main() -> int:
    print("🚀 freestyle 自动签到脚本启动")
    print(f"🕒 执行时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    app_config = AppConfig.load_from_env()
    provider = app_config.get_provider("freestyle")
    if not provider:
        msg = "❌ Provider 'freestyle' 未加载：请通过 PROVIDERS 注入/覆盖该站点配置"
        print(msg)
        _notify_fatal("freestyle 签到告警", msg)
        return 1

    accounts, accounts_err = _load_accounts()
    if not accounts:
        _notify_fatal("freestyle 签到告警", accounts_err or "❌ ACCOUNTS_FREESTYLE 未配置或格式不正确")
        return 1
    ...
    notiffreestyleion_lines: list[str] = []

    for i, account_config in enumerate(accounts):
        account_name = account_config.get_display_name(i)
        if notiffreestyleion_lines:
            notiffreestyleion_lines.append("-------------------------------")
        ...
        try:
            ...
            if ok_any:
                ...
                if display:
                    notiffreestyleion_lines.append(f"✅ {account_name}: {display}")
                else:
                    notiffreestyleion_lines.append(f"✅ {account_name}: 🏃‍♂️{quota} | Used 🏃‍♂️{used}")
                ...
                notiffreestyleion_lines.append(f"❌ {account_name}: {err or '签到失败'}")
        except Exception as e:
            any_failed = True
            notiffreestyleion_lines.append(f"❌ {account_name}: Exception: {str(e)[:160]}")
    ...
    if need_notify and notiffreestyleion_lines:
        summary = [
            "-------------------------------",
            "📢 freestyle 签到统计:",
        ...
        content = "\n\n".join(
            [
                f"🕓 执行时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                "\n".join(notiffreestyleion_lines),
                "\n".join(summary),
            ]
        )
        title = "freestyle 签到成功" if success_count == total_count else "freestyle 签到告警"
        print(content)
        notify.push_message(title, content, msg_type="text")
